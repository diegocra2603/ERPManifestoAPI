...
public class UpdateTaxConfigurationCommandHandler : IRequestHandler<UpdateTaxConfigurationCommand, ErrorOr<TaxConfigurationDto>>
{
    ...
    public UpdateTaxConfigurationCommandHandler(
    ...
    public async Task<ErrorOr<TaxConfigurationDto>> Handle(UpdateTaxConfigurationCommand request, CancellationToken cancellationToken)
    {
        ...
        if (await _taxConfigurationRepository.ExistsAsync(t => t.Name == request.Name && t.Id != new TaxConfigurationId(request.Id) && t.AuditField.IsActive))
            return Error.Validation("TaxConfiguration.NameAlreadyExists", "Ya existe otra configuración de impuesto con ese nombre.");
        ...
        taxConfiguration.Update(request.Name, request.Percentage, (TaxType)request.TaxType,
            new AccountCatalogId(request.DebitAccountId), new AccountCatalogId(request.CreditAccountId));
